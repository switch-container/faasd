...
sudo ip netns exec fc$idx iptables -t nat -A POSTROUTING -o veth0 \
  -s 172.16.0.2 -j SNAT --to 192.168.${hi}.$((lo+2))
...
-d 192.168.${hi}.$((lo+2)) -j DNAT --to-destination 172.16.0.2

# (adds a route on the host for the clone address)
sudo ip route add 192.168.${hi}.$((lo+2)) via $veth0_ip
